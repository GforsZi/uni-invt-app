<x-app-layout>
    ...
    <div class="edit">
        ...
        <!-- Form -->
        <form action="/loan/{{ $loan[0]['ln_id'] }}/edit" method="post">
            @csrf
            @method('PUT')
        <div class="input-group mb-3">
          <select name="ln_lctn_location_id" class="form-select @error('ln_lctn_location_id') is-invalid @enderror" aria-label="Default select example">
            <option value="{{ $loanLocation[0]['location']['lctn_id']??'not have' }}" selected>{{ $loanLocation[0]['location']['lctn_name']??'not have' }}</option>
        ...
        <div class="input-group mb-3" id="descriptions-wrapper">
            @foreach ($loaningAssets as $index => $loaningAsset)
            <div class="input-group mb-2 description-row">
                ...
                <select name="asset_id[{{ $index }}]" class="form-select @error('asset_id[{{ $index }}]') is-invalid @enderror" aria-label="Default select example">
                    <option value="{{ $loaningAsset->asset->ast_id }}" selected>{{$loaningAsset->asset->ast_codename??'not have'}}</option>
          ...
          <select disabled  class="form-select @error('asset_id[{{ $loaningAssets->count() }}]') is-invalid @enderror" aria-label="Default select example">
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('ln_description') is-invalid @enderror" name="ln_description">{{$loan[0]['ln_description']}}</textarea>
            </div>
